...
public class HighwayDAO implements IHighwayDAO {
	...
	private static final String  FIND_ALL_HIGHWAYNAME_STMT = "SELECT DISTINCT highway_name from Highway";
	public List<String> findAllHighwayName()throws SQLException {
		List<String> nums = new ArrayList<String>();
		try (Connection conn = ds.getConnection();
				PreparedStatement pstmt = conn.prepareStatement(FIND_ALL_HIGHWAYNAME_STMT);){
			ResultSet rs = pstmt.executeQuery();
			while(rs.next()) {
				nums.add(rs.getString("highway_name"));
			}
		}
		return nums;
	}
